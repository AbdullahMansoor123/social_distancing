...
############## Version 44 save detection results ################
    ...
    def run(self,test_video):
        ...
        while True:
            ...
            if len(results.xyxy[0]) > 0:
                ...
                for person in results.xyxy[0]:
                    ...
                    for (id1,p1),(id2,p2) in combinations(centroid_dict.items(),2):
                        ...
                        for idx in centroid_dict:
                            cx,cy= centroid_dict.get(idx)[0], centroid_dict.get(idx)[1]
                            ...
                            if idx in red_zone_list:
                                ...
                                cv2.circle(frame, (int(cx), int(cy)), 2, (0, 0, 255), 5, 1)
                            else:
                                cv2.rectangle(frame, (int(x), int(y)), (int(w), int(h)), (0, 255, 0), 1)
                                cv2.circle(frame, (int(cx), int(cy)), 2, (0, 255, 0), 5, 1)
                ...
                #
                # #draw line with people not following social distance
